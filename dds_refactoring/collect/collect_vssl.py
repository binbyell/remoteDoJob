...
def get_data_frame(now_string: str, count=0):
    ...
    result = pd.concat([pd.DataFrame({'aprtfEtryptDt': aprtfEtryptDt}),
                        ...
                        pd.DataFrame({'MS': [float(value) * initDict.MS for value in vsslGrtg]})], axis=1)
    ...
    if int(totalCount[0]) > 50+(count*50):
        ...
        print(f"count:{count+1}")
        concatResource = get_data_frame(now_string, count + 1)
        if type(concatResource) != bool:
            return pd.concat((result, concatResource), axis=0)
        else:
...
def get_vssl_with_check_1st():
    global initDict
    initDict = pb.InitDict(pb.Tag.vssl)

    print(f"MB : {initDict.MB}")
