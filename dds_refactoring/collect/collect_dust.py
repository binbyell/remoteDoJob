...
def call_dust_url(list_index, dong):
    ...
    # path
    dust_hour_csv_path = pb.make_csv_path(pb.Tag.encoding, f'{dong}', initDict, False)
    ...
    try:
        ...
        # dustHourList[0] = pb.change_hour_1_to_24(HourListDateTime, "minute")
        # print(f"collect_dust dustHourList[0] : {dustHourList[0]}")
        # ##### collect dust 첫실행 시간부가 최근수집 data에 적용되서, 업데이트가 일어나지 않는 버그 발생. 수정 요망.
        # # 다음 데이터인지 채크
        # print(f"collect_dust csv_last_datetime : {csv_last_datetime.strftime('%Y%m%d %H')}")
        # comparePinTime = pb.change_hour_1_to_24(csv_last_datetime + datetime.timedelta(hours=1), "minute")
        # print(f"call_ult_fcst_api last datetime {_timeString}")
        # print(f"call_ult_fcst_api last datetime {dustHourList[0]}")
        # if _timeString != dustHourList[0]:
        #     dustHourList = [f"{_timeString}", '', '', '', '', '', '']

    # api 에서 데이터를 받지 못한 경우
    except Exception as e:
        print(f"url make error : \n\t{url}")
        ...
        # 빈데이터 처리 여기서 수정
        dustHourList = ['' if index <= 9 else 'error' for index in range(len(Columns))]

        dustHourList[0] = pb.change_hour_1_to_24(now, "minute")
...
def get_dust():
    ...
    #     print(f"dustInfoDataFrameList - {Columns[1:][index]} : \n{dustInfoDataFrameList[index]}")

    # get csv path
    pathList = [pb.make_csv_path(encoding=pb.Tag.encoding, item=item, init_dict=initDict, is_pretreatment=True) for item in Columns[1:]]
