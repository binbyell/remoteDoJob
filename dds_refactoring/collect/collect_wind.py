...
# use with collect first
def get_data_list_from_json(json_list, init_dict):
...
def get_url(nxny) -> str:
    ...
    url = f"http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getUltraSrtNcst?serviceKey={Common.api_key}&" \
...
def call_ult_fcst_api(nxny, init_dict):
    global initDict
    url = get_url(nxny)

    # path
    wind_path = Common.make_csv_path(Common.Tag.encoding, f'{nxny[0]}_{nxny[1]}', init_dict, False)

    # call csv last index
    csv_last_index = Common.call_csv_last_index(wind_path, delta_time="hours")
    csv_last_datetime = Common.change_YMDH_to_datetime(csv_last_index)
    ...
    print(type(init_dict.Now))

    _timeString = Common.change_hour_1_to_24(init_dict.Now)

    print("call_ult_fcst_api after")

    try:
        responseText = Common.url_to_response_text(url)
        vilDataFrame = get_item(responseText)
        itemList = get_data_list_from_json(vilDataFrame, init_dict=init_dict)
        print(f"call_ult_fcst_api itemList : {itemList}")
        # change hour 01~24
        itemListDatetime = Common.change_YMDH_to_datetime(itemList[0])
        itemList[0] = Common.change_hour_1_to_24(itemListDatetime, "hour")

        # # 다음 데이터인지 채크
        comparePinTime = Common.change_hour_1_to_24(csv_last_datetime + datetime.timedelta(hours=1))
    ...
    itemDF = Common.list_data_to_data_frame_data(type_list_data=itemList, columns=columns)
    Common.update_csv(wind_path, itemDF)
...
def get_wind():
    ...
    # # 에러확인용
    # singleData = call_ult_fcst_api(nxny=nxny, init_dict=initDict)
    # # 1. nxny를 받음
    # # 2. 해당 nxny의 데이터를 csv로 저장.
    # # 3. 전처리를 위한 DataFrameList를 return
    ...
    for nxny in nxnyList:
        try:
            ...
            this_day = initDict.Now.strftime('%Y%m%d')
            path_log = f"{initDict.FolderPath}/log/{this_month}"
            path_log_txt = f"{path_log}/{this_day}_{nxny[0]}_{nxny[1]}.txt"
            Common.care_exception(e, path_log, path_log_txt)
            return False

    # get csv path
    pathList = [Common.make_csv_path(encoding=Common.Tag.encoding, item=item, init_dict=initDict, is_pretreatment=True)
    ...
    for index in range(len(pathList)):
        Common.update_csv(pathList[index], single_day_list[index])
